...
# Mount static files
# Use absolute path relative to project root (assuming main.py is in src/)
BASE_DIR = Path(__file__).resolve().parent.parent
STATIC_DIR = BASE_DIR / "static"

if not STATIC_DIR.exists():
    # Fallback or create if missing to prevent startup crash, though it should exist
...
# Initialize service lazily or ensure config is valid first
# For simplicity in this version, we initialize it globally but it will fail if env is wrong.
# With lifespan, we could initialize it there, but to keep it simple with existing code structure:
try:
    Config.validate()
    service = RecruitmentService()
except Exception as e:
    # This allows the app to be imported without crashing, but startup will fail if run directly
...
class TextRequest(BaseModel):
    ...
    def validate_text(cls, values):
        # Allow passing plain text directly instead of JSON object if possible,
        # but standard FastAPI Body expects JSON. 
        # However, to solve the "JSON decode error" from Swagger UI when pasting raw multiline text:
        # We can't easily fix the client-side JSON parsing error in Swagger UI itself.
        # But we can provide a new endpoint that accepts plain text body.
        return values
...
async def parse_resume_raw(request: Request):
    ...
    Suitable for pasting multiline text directly.
    """
    try:
...
@app.post("/api/v1/job/parse", response_model=JobDescription, summary="Parse JD Text")
...
async def submit_application(
    ...
    try:
        ...
        # If parsing fails mid-way, db writes might still have happened if not careful with transaction scope.
        # process_pdf_application commits explicitly.
        # But if process_pdf_application fails, it should raise exception and ideally not leave bad data.
        # The bad data likely comes from previous partial attempts or parallel race conditions if not handled.
        # Actually, in this case, the Resume parsing failed, so resume object wasn't created,
        # so process_pdf_application shouldn't have reached db.add().
        # Wait, the user sees "Candidate profile is completely empty".
        # This implies process_pdf_application CONTINUED even after parse_resume failed?
        # OR parse_resume returned an empty/partial object instead of raising?
        # Let's check parse_resume implementation.
        raise HTTPException(status_code=500, detail=str(e))
...
async def batch_submit_application(
    ...
    try:
        ...
        for file in files:
            if not file.filename.lower().endswith('.pdf'):
                # We could skip or error. Let's skip invalid ones or just include in error report.
                continue
